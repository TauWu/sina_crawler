...
def get_hot_pageNum(page):
	try:
		count=page.xpath("/html/body/div[5]/span/text()")[0]
...
def search_hot(s,keyword,endtime,nick='',starttime='',sort='time'):
	...
	for i in range(2,pageNum+1):
		url='https://weibo.cn/search/mblog?hideSearchFrame=&keyword=%s&advancedfilter=1&nick=%s&starttime=%s&endtime=%s&sort=%s&page=%s' \
		...
		print ('='*30)
		print ('keyword:%s starttime:%s endtime:%s cpage:%s'%(keyword,starttime,endtime,i))
		print ('='*30)
